...
export default class InternetComputerApp extends GenericApp {
    ...
    return await this.transport
      ...
      .then((response: Buffer) => {
        ...
        if (returnCode === LedgerError.NoErrors && response.length > 2) {
          ...
          signatureDER = response.subarray(
            PREHASH_LEN + SIGRSLEN + 1,
            response.length - 2,
          );
          return {
            preSignHash,
  ...
  async signUpdateCall(
    ...
    const message = Buffer.alloc(
      8 + request.byteLength + checkStatus.byteLength,
    );
    ...
    return await this.signSendChunk(
      ...
      for (let i = 1; i < chunks.length; i += 1) {
        // eslint-disable-next-line no-await-in-loop
        result = await this.signSendChunkUpdateCall(
          1 + i,
          chunks.length,
          chunks[i],
          txtype % 256,
  ...
  async sendData(
    ...
    return await this.sendChunk(1, chunks.length, chunks[0], instruction).then(
      async (response) => {
        let result = {
          returnCode: response.returnCode,
          errorMessage: response.errorMessage,
  ...
  async sendCertificateAndSig(
    ...
    return await this.signSendChunk(
      ...
      chunks[0],
      0,
      this.INS.SAVE_CERITIFACE_AND_VERIFY,
    ).then(async (response) => {
      let result = {
        returnCode: response.returnCode,
        errorMessage: response.errorMessage,
  ...
  async tokenRegistry(): Promise<ResponseTokenRegistry> {
    ...
    if (
      registrySize.returnCode !== LedgerError.NoErrors ||
    ...
    for (let i = 0; i < registrySize.RegistrySize; i += 1) {
      ...
      if (
        ...
        response.returnCode !== LedgerError.NoErrors
      ) {
        return {
